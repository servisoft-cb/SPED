...
uses rsDBUtils, StdConvs, DmdDatabase;
...
begin
  ...
  begin
    MessageDlg('*** Código do Leiaute não informado!', mtInformation, [mbOk], 0);
    exit;
  end;
  if trim(fDMSpedVersao.cdsSpedVersaoVERSAO_LEIAUTE.AsString) = '' then
  begin
    MessageDlg('*** Versão do Leiaute não informado!', mtInformation, [mbOk], 0);
    exit;
  end;

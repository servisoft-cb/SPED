...
begin
  vComando    := 'select AUX.*,'
               ...
               + '         else ' + QuotedStr('')
               + '       end DESC_TIPO_REG, '
               + '       iif(Em.TIPO_ES = ' + QuotedStr('E') + ' and em.GERAR_CUSTO = ' + QuotedStr('S') + ', cast(sum(em.QTD * em.VLR_UNITARIO),0) as Numeric(15,5)) VLR_ENTRADA, '
               + '       iif(Em.TIPO_ES = ' + QuotedStr('E') + ' and em.GERAR_CUSTO = ' + QuotedStr('S') + ',cast(sum(em.QTD),0) as numeric(15,5)) as QTD_ENTRADA, PRO.PERC_IPI, pro.id_ncm, PRO.perc_icms '
               + 'from PRODUTO pro '
               ...
               + 'a as '
               + '( '
               + 'select E.ID_PRODUTO, E.TAMANHO, cast(sum(e.QTD_ESTOQUE) as numeric(15,5)) as QTD_ESTOQUE, e.REFERENCIA, '
               + '       e.NOME_PRODUTO, e.UNIDADE, e.ID_COR, e.SPED_TIPO_ITEM, e.TIPO_REG, '
               + '       e.TIPO_SPED, '
               + '       e.DESC_TIPO_REG, '
               + '       cast(sum(e.VLR_ENTRADA) as numeric(15,5)) as VLR_ENTRADA , '
               + '       cast(sum(e.QTD_ENTRADA) as numeric(15,5)) as QTD_ENTRADA, e.PERC_IPI, '
